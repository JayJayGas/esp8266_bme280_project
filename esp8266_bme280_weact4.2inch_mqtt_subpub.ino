...
*
* E-Paper details:
* 4.2'' WeACT E-paper red and black:
...
#define GRID_ARRAY_SIZE 4           //number of times you split screen into grid, must be even.
...
///////////////////Proto////////////////////////////
//std funcs for good programming
...
void setup(void) {
  //init error handling
  if (GRID_ARRAY_SIZE % 2 != 0){
    Serial.println("Definition GRID ARRAY SIZE Definition must be even");
      exit(EXIT_FAILURE);
    }
  if (GRID_ARRAY_SIZE <= (NUMBER_OF_SENSORS )){
    Serial.println("Grid Array Size must be more than the NUMBER OF SENSORS Definition.");
    Serial.println("One grid section used for time and date data.");
    exit(EXIT_FAILURE);
  }
...
////////////////////SETUP FUNCS//////////////////////////
/*****************************************************
* @brief standard safe malloc call wrapper
* @param size: size of memory allocation.
*****************************************************/
void *safe_malloc(size_t size) {
  void *ptr = malloc(size);
  if (ptr == NULL && size > 0) {
    Serial.println("safe_malloc failed to allocate memory.");
    exit(EXIT_FAILURE);
  }
  return ptr;
}
...
struct shape_size* ret_sub_grid_size(struct shape_size shape) {
  struct shape_size* ret_grid_box = (shape_size*)safe_malloc(GRID_ARRAY_SIZE * sizeof(shape_size));
...
struct shape_size* text_size(const char* string_array[], uint16_t array_size) {
  ...
  struct shape_size* text_size = (shape_size*)safe_malloc(array_size * sizeof(shape_size));
  //vars for loop
  int16_t tbx; int16_t tby;
  uint16_t tbw; uint16_t tbh;
  uint16_t x; uint16_t y;
...
*****************************************************/
int print_screen_all() {
  //funcs to generate dimensional data/coords
  struct shape_size screen_grid_size = ret_grid_size(display.width(), display.height());
  struct shape_size* screen_sub_grid_size = ret_sub_grid_size(screen_grid_size); //malloc func - free!
  struct shape_size* display_headings_size = text_size(display_headings,
                                                      display_headings_array_size); //malloc func - free!
  struct shape_size* data_headings_size = text_size(data_headings,
                                                    data_headings_array_size); //malloc func - free!
  struct shape_size* data_array_size  = text_size(data_array,
                                            (NUMBER_OF_SENSORS*NUMBER_OF_DATA_POINTS_FROM_SENSORS+2)); //malloc func - free!
  ...
  /****************MALLOC FREE************/
  free(screen_sub_grid_size);  //ret_grid_box_size()
